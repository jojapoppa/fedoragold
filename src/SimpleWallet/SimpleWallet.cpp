...
std::string getExistingWalletFileName(Config &config) {
    ...
    while (true) {
        ...
        } else if (boost::filesystem::exists(walletName)) { /* Allow people to enter wallet name with or without file extension */
            return walletName;
        } else if (boost::filesystem::exists(walletFileName)) {
            return walletFileName;
        } else {
            std::cout << WarningMsg("A wallet with the filename ") << InformationMsg(walletFileName) << WarningMsg(" doesn't exist!") << std::endl
...
std::string getNewWalletFileName() {
    ...
    while (true) {
        ...
        if (boost::filesystem::exists(walletFileName)) {
            std::cout << WarningMsg("A wallet with the filename ") << InformationMsg(walletFileName) << WarningMsg(" already exists!") << std::endl
...
void promptSaveKeys(CryptoNote::WalletGreen &wallet) {
    ...
    printPrivateKeys(wallet, false, true);
...
void exportKeys(std::shared_ptr<WalletInfo> &walletInfo) {
    ...
    printPrivateKeys(walletInfo->wallet, walletInfo->viewWallet, false);
}
...
void printPrivateKeys(CryptoNote::WalletGreen &wallet, bool viewWallet, bool isNewWallet) {
    ...
    if (!isNewWallet) {
    std::cout << std::endl
              << InformationMsg("Keys for address:") << std::endl
              << SuccessMsg(walletAddress) << std::endl;

    }
    std::cout << std::endl
              << SuccessMsg("--------------------------- CLI KEYS ---------------------------") << std::endl
              << InformationMsg("Secret spend key:") << std::endl
              << Common::podToHex(privateSpendKey) <<std::endl
              << InformationMsg("Secret view key:") << std::endl
              << Common::podToHex(privateViewKey) << std::endl
              << SuccessMsg("--------------------------- GUI KEY ----------------------------") << std::endl
    ...
    if (deterministicPrivateKeys) {
        ...
        std::istringstream iss(mnemonicSeed);
        for(std::string mnemonicSeed; iss >> mnemonicSeed; ) {
            seedWords.push_back(mnemonicSeed);
        }

        std::cout << SuccessMsg("------------------------ MNEMONIC SEED -------------------------") << std::endl
                  << InformationMsg("1) ") << seedWords[0] << (seedWords[0].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("10) ") << seedWords[9]  <<  (seedWords[9].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("19) ") << seedWords[18] << std::endl
                  << InformationMsg("2) ") << seedWords[1] << (seedWords[1].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("11) ") << seedWords[10] << (seedWords[10].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("20) ") << seedWords[19] << std::endl
                  << InformationMsg("3) ") << seedWords[2] << (seedWords[2].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("12) ") << seedWords[11] << (seedWords[11].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("21) ") << seedWords[20] << std::endl
                  << InformationMsg("4) ") << seedWords[3] << (seedWords[3].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("13) ") << seedWords[12] << (seedWords[12].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("22) ") << seedWords[21] << std::endl
                  << InformationMsg("5) ") << seedWords[4] << (seedWords[4].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("14) ") << seedWords[13] << (seedWords[13].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("23) ") << seedWords[22] << std::endl
                  << InformationMsg("6) ") << seedWords[5] << (seedWords[5].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("15) ") << seedWords[14] << (seedWords[14].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("24) ") << seedWords[23] << std::endl 
                  << InformationMsg("7) ") << seedWords[6] << (seedWords[6].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("16) ") << seedWords[15] << (seedWords[15].length() < 4 ? "\t\t\t" : "\t\t") << InformationMsg("25) ") << seedWords[24] << std::endl
                  << InformationMsg("8) ") << seedWords[7] << (seedWords[7].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("17) ") << seedWords[16] << std::endl
                  << InformationMsg("9) ") << seedWords[8] << (seedWords[8].length() < 5 ? "\t\t\t" : "\t\t") << InformationMsg("18) ") << seedWords[17] << std::endl;
    } 
...
void inputLoop(std::shared_ptr<WalletInfo> &walletInfo, CryptoNote::INode &node) { 
    while (true) {
        ...
        if (command == "") {
            // no-op
        } else if (command == "export_keys" || command == "keys") {
        ...
        } else if (command == "incoming_transfers" || command == "incoming") {
        ...
        } else if (words[0] == "change_password" || words[0] == "change password" || words[0] == "password") {
            words.erase(words.begin());
            changePassword(walletInfo, words);
        } else if (!walletInfo->viewWallet) {
            if (command == "outgoing_transfers" || command == "outgoing") {
                listTransfers(false, true, walletInfo->wallet, node);
            } else if (command == "list_transfers" || command == "transfers") {
...
void blockchainHeight(CryptoNote::INode &node, CryptoNote::WalletGreen &wallet) {
    ...
    if (localHeight == remoteHeight) {
        std::cout << SuccessMsg(std::to_string(localHeight + 1));
    } else {
        std::cout << WarningMsg(std::to_string(localHeight + 1));
    }
...
void printOutgoingTransfer(CryptoNote::WalletTransaction t, CryptoNote::INode &node) {
    std::string blockTime = unixTimeToDate(t.timestamp);

    std::cout << std::endl
              << InformationMsg("Outgoing transfer") << std::endl;
    if (blockTime != "") {
        std::cout << WarningMsg("Time: " + blockTime) << std::endl;
    }
    std::cout << WarningMsg("Hash: " + Common::podToHex(t.hash)) << std::endl
...
void printIncomingTransfer(CryptoNote::WalletTransaction t, CryptoNote::INode &node) {
    ...
    /* Couldn't get timestamp, maybe old node or the daemon closed */
    if (blockTime != "") {
...
void findNewTransactions(CryptoNote::INode &node, std::shared_ptr<WalletInfo> &walletInfo) {
    ...
    if (walletHeight == 1) {
        std::cout << "Scanning through the blockchain to find transactions that belong to you." << std::endl
                  << "Please wait, this will take some time. (Use CTRL+C if you have to cancel)" << std::endl 
                  << std::endl;
    } else {
