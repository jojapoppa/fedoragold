...
bool core::check_tx_inputs_keyimages_diff(const Transaction& tx) {
  ...
  for (const auto& input : tx.inputs) {
    if (input.type() == typeid(KeyInput)) {
	  ...
	  if (!(scalarmultKey(in.keyImage, (const Crypto::KeyImage&)LL) == (const Crypto::KeyImage&)II)) {
...
CryptoNote::tx_memory_pool::PoolTransactionDetails core::getTransactionDetails(Crypto::Hash txid) const {
  std::list<CryptoNote::tx_memory_pool::PoolTransactionDetails> tpool;
  m_mempool.getMemoryPool(tpool);
  ...
  // returns a blank on error
  CryptoNote::tx_memory_pool::PoolTransactionDetails outp = {};
  return outp;
}
