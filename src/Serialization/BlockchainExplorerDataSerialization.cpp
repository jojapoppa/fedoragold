...
void serialize(TransactionInputMultisignatureDetails& inputMultisig, ISerializer& serializer) {
  serializer(inputMultisig.signatures, "input");
  serializer(inputMultisig.output, "output");
}

/* FED only has 1 variant
void serialize(TransactionInputMultisignatureDetails& input, ISerializer& serializer) {
  if (serializer.type() == ISerializer::OUTPUT) {
    BinaryVariantTagGetter tagGetter;

    uint8_t tag = static_cast<uint8_t>(SerializationTag::Multisignature);
