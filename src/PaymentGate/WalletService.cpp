...
std::string createTemporaryFile(const std::string& path, std::fstream& tempFile) {
  bool created = false;
  std::string temporaryName = path + ".backup";
  try {deleteFile(temporaryName);} catch(...) { /* just eat the error, it may not exist yet */ }
...
WalletService::WalletService(const CryptoNote::Currency& currency, System::Dispatcher& sys, CryptoNote::INode& node, CryptoNote::IWallet& wallet, CryptoNote::IFusionManager &fusionManager, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
...
std::error_code WalletService::saveWalletNoThrow() {
  try {
    ...
    std::string ermsg = "Error while saving wallet: ";
    ermsg += x.what();
    logger(Logging::WARNING, Logging::BRIGHT_YELLOW) << ermsg;
...
std::error_code WalletService::resetWallet() {
  try {
    ...
    std::string ermsg = "Error while reseting wallet: ";
    ermsg += x.what();
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    std::string ermsg = "Error while replacing container: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
  }
...
std::error_code WalletService::getTransactionHashes(const std::vector<std::string>& addresses, const std::string& blockHashString,
  ...
  try {
    ...
    logger(Logging::WARNING) << "Error while getting transactions for block RPC Info: " << x.what();
    return x.code();
  } catch (std::exception& x) {
    std::string ermsg = "Error while getting transaction hashes: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getTransactionHashes(const std::vector<std::string>& addresses, uint32_t firstBlockIndex,
  ...
  try {
    ...
    logger(Logging::WARNING) << "Error while getting transactions for hashes: " << x.what();
    return x.code();
  } catch (std::exception& x) {
    std::string ermsg = "Error while getting transaction hashes by index: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getTransactions(const std::vector<std::string>& addresses, const std::string& blockHashString,
  ...
  try {
    ...
    logger(Logging::WARNING) << "System error while getting transactions: " << x.what();
    return x.code();
  } catch (std::exception& x) {
    std::string ermsg = "Error while getting transactions: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getTransactions(const std::vector<std::string>& addresses, uint32_t firstBlockIndex,
  ...
  try {
    ...
    logger(Logging::DEBUGGING) << "Sys error while getting transactions: " << x.what();
    return x.code();
  } catch (std::exception& x) {
    std::string ermsg = "Error while getting transactions with addresses: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    std::string ermsg = "Error while getting transaction: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getTransactionProofNoLock(const std::string& transactionHash, const std::string& destinationAddress, const std::string& transactionSecretKey, std::string& transactionProof) {
  try {
    Crypto::Hash hash = parseHash(transactionHash, logger);
    Crypto::SecretKey txSecretKey = wallet.getTransactionSecretKey(hash);
...
std::error_code WalletService::getAddresses(std::vector<std::string>& addresses) {
  try {
    ...
    std::string ermsg = "Can't get addresses: ";
    ermsg += e.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash, std::string& proof) {
  try {
    System::EventLock lk(readyEvent);
    ...
    Crypto::Hash ahash = wallet.getTransaction(transactionId).hash;
    transactionHash = Common::podToHex(ahash);
    if (sendParams.destinations.size() == 1) dest = sendParams.destinations.front().address;
    Crypto::SecretKey aSecretKey = wallet.getTransactionSecretKey(ahash);
    std::string transactionSecretKey = Common::podToHex(aSecretKey);

    // Automatically generate proof for all single destination outgoing transfers
    if ((aSecretKey != CryptoNote::NULL_SECRET_KEY) && (request.transfers.size() == 1)) {
      getTransactionProofNoLock(transactionHash, dest, transactionSecretKey, proof);
    }
    ...
    std::string ermsg = "Error while sending transaction: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::createDelayedTransaction(const CreateDelayedTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    std::string ermsg = "Error while creating delayed transaction: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getDelayedTransactionHashes(std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::string ermsg = "Error while getting delayed transaction hashes: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    std::string ermsg = "Error while deleting delayed transaction hashes: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::sendDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    std::string ermsg = "Error while sending delayed transaction hashes: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::bindDaemon(const std::string& daemonIP, const std::string& daemonPort) {
  ...
  try {
    ...
    std::string ermsg = "Error while switching to new daemon: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::string ermsg = "Error while getting unconfirmed transaction hashes: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::getStatus(uint32_t& blockCount, uint32_t& knownBlockCount, uint32_t& localDaemonBlockCount, std::string& lastBlockHash, uint32_t& peerCount) {
  try {
    ...
    std::string ermsg = "Error while getting status: ";
    ermsg += x.what();
    logger(Logging::WARNING) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::sendFusionTransaction(uint64_t threshold, uint32_t anonymity, const std::vector<std::string>& addresses, const std::string& destinationAddress, std::string& transactionHash) {
  ...
  try {
    ...
    std::string ermsg = "Error while sending fusion transaction: ";
    ermsg += x.what();
    logger(Logging::WARNING, Logging::BRIGHT_YELLOW) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
...
std::error_code WalletService::estimateFusion(uint64_t threshold, const std::vector<std::string>& addresses, uint32_t& fusionReadyCount, uint32_t& totalOutputCount) {
  ...
  try {
    ...
    std::string ermsg = "Failed to estimate number of fusion outputs: ";
    ermsg += x.what();
    logger(Logging::WARNING, Logging::BRIGHT_YELLOW) << ermsg;
    return make_error_code(CryptoNote::error::INTERNAL_WALLET_ERROR, ermsg);
  }
