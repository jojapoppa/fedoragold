...
namespace {
  ...
  const command_line::arg_descriptor<uint16_t>    arg_rpc_port  = {"walletd-port", "RPC port of walletd. Default: 30160", 30160};
...
bool validAddress(po::variables_map& vm, const std::string& address) {
  ...
  try {
    HttpClient httpClient(dispatcher, command_line::get_arg(vm, arg_ip), command_line::get_arg(vm, arg_rpc_port), logger);
