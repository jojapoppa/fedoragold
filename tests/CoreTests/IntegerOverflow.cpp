...
bool gen_uint_overflow_2::generate(std::vector<test_event_entry>& events) const
{
  ...
  if (!constructTransaction(miner_account.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tx_1, 0, tx_key, m_logger.getLogger()))
    return false;
  ...
  if (!constructTransaction(bob_account.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tx_2, 0, tx_key, m_logger.getLogger()))
    return false;
